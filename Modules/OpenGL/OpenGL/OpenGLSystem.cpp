...
void OpenGLSystem::onInitialize(ECS::Registry& registry, World& world)
{
    ...
    const ECS::EntityId openglConfigId = ResourceSystem::loadSync<OpenGLConfig>(registry, openglConfigRelPath);
...
void Mani::OpenGLSystem::loadAndCompileShadersSync(ECS::Registry& registry, World& world)
{
    ...
    const ECS::EntityId configId = ResourceSystem::loadSync<ShaderConfig>(registry, configPath);
    ...
    const ECS::EntityId collectionId = ResourceSystem::loadSync<ShaderCollection>(registry, collectionPath);
    ...
    {
        // load shader
        ResourceSystem::loadSync<Shader>(registry, shaderPath);
    }
}