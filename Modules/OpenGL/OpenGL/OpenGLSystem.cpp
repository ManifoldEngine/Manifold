...
void Mani::OpenGLSystem::loadAndCompileShadersSync(ECS::Registry& registry, World& world)
{
    ...
    const ShaderConfig& config = registry.get<Resource<ShaderConfig>>(configId)->get();
    ...
    const ShaderCollection& collection = registry.get<Resource<ShaderCollection>>(collectionId)->get();
