...
bool ResourceLoader_Font::load(ECS::Registry& registry, const Path& absolutePath, EntityId resourceId, uint32_t tag) const
{
	Resource<Font>& res = registry.getPinned<Resource<Font>>(resourceId);
	return STB::loadFont(absolutePath, res.value);
}

bool ResourceLoader_Font::unload(ECS::Registry& registry, EntityId resourceId) const
{
	Resource<Font>& res = registry.getPinned<Resource<Font>>(resourceId);
